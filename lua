...
do
    local window = library:CreateWindow('Funky Friday') do
        local folder = window:AddFolder('Autoplayer') do
            local toggle = folder:AddToggle({ = 'Autoplayer', flag = 'autoPlayer' })
        ...
        local folder = window:AddFolder('Credits') do
            folder:AddLabel({ text = 'AceZ - UI library' })
            folder:AddLabel({ text = 'Germany Driving - Script' })
        end
        ...
        window:AddButton({ text = 'Copy discord', callback = function()
              setclipboard("https://discord.gg/njvPVvY7")
        end })
        window:AddDivider()
